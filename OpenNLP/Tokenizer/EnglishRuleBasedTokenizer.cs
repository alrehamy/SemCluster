...
namespace OpenNLP.Tokenizer
{
    ...
    {
        private static readonly Regex UriRegex = new Regex("[(?://]([\\w-]+\\.)+[\\w-]+(/[\\w- ./?%&=]*)?", RegexOptions.Compiled | RegexOptions.IgnoreCase);
        ...
        public override Span[] TokenizePositions(string input)
        {
            ...
            // split on spaces
            List<Span> parts = new List<Span>();
            int currentStartOfSpan = 0;
        ...
        {
            string token = input.Substring(span.Start, span.Length());

            if (span.Length() <= 1)
                return new List<Span>() { span };

            if(LettersOnlyRegex.IsMatch(token))
                return new List<Span>() { span };

            if (UriRegex.IsMatch(token))
                return new List<Span>() { span };

            string[] splitTokens = _tokenizationRegex.Split(token);
